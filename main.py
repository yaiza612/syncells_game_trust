...
def main(production, step, size, initial_density, cooperation, threshold_survival, threshold_reproduction, initial_QS,
         diffusion_factor, path_images_simulation, path_additional_figures, name_video, timestamp):
    medium = create_medium(size, initial_density)
    selfish_and_cooperative = create_selfish_and_cooperative(medium, cooperation)
    quorum_signals_initial = np.ones((medium.shape[0] - 1, size, size)) * initial_QS
    initial_savings = create_savings(size)
    q_signal = quorum_signals_initial
    mediums, q_signals, savings = run_simulation(medium, initial_savings, q_signal, selfish_and_cooperative,
                                                   production, diffusion_factor,
                                                   threshold_survival, threshold_reproduction, step)
    make_fig(mediums, q_signals, savings, path_images_simulation, path_additional_figures, name_video, timestamp)
...
if __name__ == "__main__":
    ...
    main(production=args.production, step=args.step, size=args.size, initial_density=args.initial_density,
         cooperation=args.cooperation, threshold_survival=args.threshold_survival,
         ...
         path_additional_figures=args.path_additional_figures, name_video=args.name_video,
